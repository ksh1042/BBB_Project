...
<!--이슈닫기 아이콘  -->
<!-- <li><i class="fa fa-fw fa-ban" style="background-color:#FF0000;color:white;"></i> -->
							<div class="timeline-item">
								...
								<c:if test="${loginUser.id eq issue.writer }">
									<div class="timeline-footer" style="height: 50px;">
										<button type="button" class="btn btn-primary btn-xs" style="padding: 5px 10px 5px 10px; float: right;" id="issueModifyBtn">수정</button>
						...
						<c:if test="${issue.openyn eq 1}">
							<li>
								<i class="fa fa-fw fa-ban" style="background-color:#dd4b39;color:white;"></i>
								<div class="timeline-item">
									<div class="timeline-body" style="text-align:center;"><h4><strong>이슈가 닫혔습니다.</strong></h4>
									<c:if test="${loginUser.id eq issue.writer }">
										<form action="open" method="post" name="openIssueForm">
											<input type='hidden' name='iNum' value="${issue.iNum}">
											<button type="button" class="btn btn-primary btn-xs" style="padding: 5px 10px 5px 10px;margin-top:6px;margin-bottom:3px;" id="openBtn">이슈 열기</button>
										</form>
									</c:if>
									</div>
								</div>
			...
			<c:if test="${issue.openyn eq 0}">
				<div class="box box-primary" id="detailBlock">
					...
					<div class="box-footer">
						<c:if test="${loginUser.id eq issue.writer }">
								<button type="button" id="closeBtn" class="btn btn-default" style="margin-left: 930px;">이슈 닫기</button>
						</c:if>
								<button type="button" class="btn btn-primary pull-right" onclick="addIssueDetail_go()">등록</button>
							<form name="closeForm">
								<input type='hidden' name='iNum' value="${issue.iNum}"> 
							</form>
					</div>
           ...
           <div class="timeline-body div-detail">
			...
			<div id="issueDetailModifyBtnDiv">
				<div class="timeline-footer" style="height: 50px;" id="detailModifyBtn">
					<button class="btn btn-primary btn-xs btn-modify"
					style="padding:5px 10px 5px 10px;margin-left:910px;">
					수정
					</button>
...
<script>
	...
	$("#issueModifyBtn").on("click", function() {
	 ...
	 $.each("${issueDetail}",function(index,item){
		var writer= item.writer;
		
		if(loginUser == writer){
			$('#detailModifyBtn').attr('type',hidden);
		}
	});  
	
	
	/* for(var detail in "${issueDetail}"){
		detailWriter[detail] = "${detail.writer}";
	}
	
	for(var i=0;i<detailWriter.lengh;i++){
		console.log(detailWriter[i]);
	} */
	
	/* <c:forEach var="detail" items="${issueDetailList}">
		'${detail.writer}'
	</c:forEach> */
	...
	Handlebars.registerHelper('isWriter', function(options) {
		
		var loginUser ="${loginUser.id}"; 
	  	
		
		
	 	$.each(detailWriter,function(index,item){
			var writer= item.writer;
	...
	/* 	<c:forEach var="detail" items="${issueDetailList}">
			<c:if test="loginUser == '${detail.writer}' ">
				return options.fn(this);
			</c:if>
	...
	/* 	$.each($("#writerList"),function(index,item){
			var writer= item.writer;
...
/* 
		<c:forEach var="detail" items="${detailList}">
			detailWrite = '${detail.writer}';	

			if(loginUser == detailWriter){
				return options.fn(this);
			}
